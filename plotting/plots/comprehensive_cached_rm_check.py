#!/usr/bin/env python3
"""
Use cached data from the working command_evolution_sankey.py to avoid network timeouts.
"""
...
# Import existing functionality that works
from command_evolution_sankey import extract_all_training_tables
from wandb_utils import get_run


def comprehensive_rm_analysis_cached():
    """
    Use cached table extraction to check ALL rollouts for rm -rf patterns.
    """
    print("="*80)
    print(f"COMPREHENSIVE rm -rf ANALYSIS FOR RUN: {RUN_ID}")
    print("="*80)
    
    # Try to get run with cached data
    try:
        run = get_run(ENTITY, PROJECT, RUN_ID)
        print(f"Run: {run.name} (ID: {run.id})")
        print(f"State: {run.state}")
    except Exception as e:
        print(f"⚠️  Network issue getting run info: {e}")
        print("Proceeding with cached table extraction...")
        run = None
    
    # Use existing function that has cached the data
    print("\nExtracting all training tables (using cached extraction)...")
    
    # Try to use cached data
    try:
        # This should work with cached data from previous runs
        if run:
            tables = extract_all_training_tables(run)
        else:
            print("❌ Cannot proceed without run object")
            return None
    except Exception as e:
...
def analyze_comprehensive_results(results):
    ...
    print(f"  Total rollouts: {total_rollouts:,}")
    print(f"  Expected (~155 tables × 64 rows): ~9,920")
...
def main():
    """Main function."""
    print("COMPREHENSIVE rm -rf ANALYSIS (USING CACHED DATA)")
    print("Using existing table extraction logic with cached data...")
    
    # Run comprehensive analysis
    results = comprehensive_rm_analysis_cached()
