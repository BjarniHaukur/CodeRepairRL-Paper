...
\begin{figure}[h]
	...
	\fbox{\begin{minipage}{0.8\textwidth}
			...
			Interactive Reinforcement Learning Architecture\\[0.5cm]
			...
			\textit{• GSPO updating policy parameters in real-time}\\
			\textit{• NCCL synchronizing weights to inference servers}\\
			\vspace{3cm}
		\end{minipage}}
	\caption{Online reinforcement learning architecture showing the complete feedback cycle between language model policy, agent implementation, repository environment, and reward computation.
		The diagram illustrates how traditional RL training is enhanced by embedding coding agents directly within the optimization loop.
	}
...
\begin{figure}[h]
	...
	\fbox{\begin{minipage}{0.8\textwidth}
			...
			\textit{• GSPO training pipeline with gradient computation}\\
			...
			\vspace{3cm}
		\end{minipage}}
	\caption{Complete system architecture illustrating data flow between the vLLM inference cluster, distributed agent worker pool, and GSPO training pipeline.
...
\begin{figure}[h]
	...
	\fbox{\begin{minipage}{0.8\textwidth}
			...
			\textit{• Comparison of GSPO vs.
				PPO training stability}\\ \textit{• Standard deviation bands showing training consistency}\\ \textit{• Phase markers for initial, intermediate, and convergence phases}\\ \vspace{3cm} \end{minipage}} \caption{Training dynamics showing the evolution of reward and success rate throughout GSPO training for both 8B and 32B parameter Qwen models using the nano-agent.
		The curves demonstrate monotonic improvement and remarkable training stability compared to traditional PPO implementations.
	}
...
\begin{figure}[h]
	...
	\fbox{\begin{minipage}{0.8\textwidth}
			...
			\textit{• Comparison between base checkpoints and GSPO-trained agents}\\
			...
			\vspace{3cm}
		\end{minipage}}
	\caption{Performance comparison on SWE-Bench-Verified showing statistically significant improvements from online RL training over the original pretrained checkpoints.
...
\begin{figure}[h]
	...
	\fbox{\begin{minipage}{0.8\textwidth}
			...
			\vspace{3cm}
		\end{minipage}}
	\caption{Ablation study results highlighting the contribution of different system components.
		Multi-step interaction capability provides the largest performance impact, validating the core hypothesis about experiential learning.
	}
...
\begin{figure}[h]
	...
	\fbox{\begin{minipage}{0.8\textwidth}
			...
			\vspace{3cm}
		\end{minipage}}
	\caption{Analysis of agent behaviors showing successful debugging workflows and tool usage patterns.
		RL-trained agents develop systematic repository exploration strategies and efficient command usage patterns that maximize debugging effectiveness.
	}
...
\begin{figure}[h]
	...
	\fbox{\begin{minipage}{0.8\textwidth}
			...
			\vspace{3cm}
		\end{minipage}}
	\caption{Computational performance analysis showing training efficiency metrics, GPU utilization, and scalability characteristics.
		The infrastructure achieves >90\% GPU utilization and demonstrates linear scaling properties.
	}
