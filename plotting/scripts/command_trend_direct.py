...
def get_command_metrics_from_history(run, ema_alpha=0.05):
    """
...
def create_trend_plot(ratio_df, run_name, filename):
    ...
    for cmd in TOP_COMMANDS:
        cmd_data = ratio_df[ratio_df['command'] == cmd].sort_values('step')
        if len(cmd_data) > 0:
            color = get_command_color(cmd)
            
            # Special styling for apply_patch
            if cmd == 'apply_patch':
                ax.plot(cmd_data['step'], cmd_data['ratio'], 
    ...
    ax.set_title(f'{run_name}: Command Usage Trends During Training', 
...
def main():
    ...
    ratio_df = get_command_metrics_from_history(run, ema_alpha=args.ema_alpha)
