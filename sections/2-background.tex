...
Having established why agentic systems enable \ac{RL} training, we provide an in-depth treatment of policy optimization algorithms, progressing from \ac{PPO} through group-relative methods, with emphasis on stability challenges.
We then cover parameter-efficient adaptation, instruction-driven datasets, and conclude with research positioning that contrasts our execution-free approach with concurrent work, establishing the methodological context for subsequent chapters.
...
With the theoretical and practical foundations established, we now examine the datasets used to train and evaluate code repair systems in realistic settings.

\section{Instruction-Driven Repository-Level Datasets}
\label{sec:training-evaluation-datasets}

Instruction-driven datasets provide natural language issue descriptions alongside buggy repository states and ground-truth patches.
...
\textbf{DeepSWE}~\cite{deepSWE2025} validates the viability of online \ac{RL} for terminal-based coding agents, applying \ac{GRPO} to a 32B parameter model with asynchronous inference infrastructure across 72 H100 \acp{GPU} (64 for training, 8 for inference).
Their work demonstrates that multi-turn agent training can achieve strong performance with test-based rewards.
In contrast, our work demonstrates that similar training methodologies remain tractable within academic compute constraints.
Our infrastructure supports models up to 32B parameters on 6 A100 \acp{GPU}; primary experiments reported here use 3 A100 \acp{GPU} with a 14B parameter model.

We adopt execution-free patch-similarity rewards, building on the patch-based signal introduced by SWE-RL~\cite{wei2025swerladvancingllmreasoning}, rather than test-based evaluation.
This choice reduces engineering friction—avoiding per-repository test harnesses and heavyweight containerization—and enables concurrent training across heterogeneous environments and programming languages without language-specific execution infrastructure.
Detailed design considerations are deferred to the methods chapter.
...
\textbf{Minimalist agent approach}: We demonstrate that a simple agent design with basic tools can achieve strong performance when combined with reinforcement learning, validating that complex debugging behaviors develop through learning rather than engineered tool sophistication.
