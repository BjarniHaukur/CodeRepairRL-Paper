...
Complementary techniques for scalable training (parameter-efficient adaptation via \ac{LoRA} and sampling choices that balance exploration and exploitation) are summarized alongside evaluation resources such as SWE-Bench and TauBench.
...
Since then, frontier labs have converged on repository-grounded evaluation (e.g., SWE-Bench Verified) and increasingly ship first-party \emph{agent harnesses} such as OpenAI's Codex \ac{CLI}, Anthropic's Claude Code, and Google's Gemini \ac{CLI} that orchestrate tool calls over live repositories~\cite{sweBench2024,sweBenchVerified2024,openaiCodexCLIRepo,claudeCode,geminiCLIRepo}.
...
This section reviews policy optimization algorithms for \acp{LLM}, progressing from \ac{PPO} through group-relative methods (\ac{GRPO}, Dr.
\,GRPO, \ac{DAPO}) to \ac{GSPO}, which we ultimately adopt for its superior robustness in multi-turn agent training.
We first establish \ac{PPO} notation, then present the group-relative baseline principle, and conclude with \ac{GSPO}'s sequence-level importance weighting that addresses stability challenges in variable-length trajectories.
...
>\! 0$.
\todoinline{give this one pass to ensure correctness / clarity with final agorithm seutp}
We omit the customary \ac{KL} regularizer to a reference model both in practice and for clarity.
...
\end{equation} the surrogate mirrors \ac{PPO}'s clipped form while averaging first within a sequence and then across the group: \begin{equation}
J_{\mathrm{GRPO}}(\theta)
\;=\;
...
\ac{GRPO} preserves the \ac{PPO} update logic but substitutes a per query group baseline for the value model, which removes the critic, lowers memory, and simplifies training while keeping the clipped ratio safeguard and on policy sampling \cite{grpo2024}.
...
\Big)
\right],
\end{equation} with $A_i$ the group standardized advantage as in \ac{GRPO} and asymmetric clipping $(\epsilon_{\text{low}},\epsilon_{\text{high}})$.
...
First is aligning the importance weighing to sequence-level advantages and better handling of small logit discrepancies between training and inference engines accumulate across long sequences.
Token-level importance weighting in \ac{GRPO} is fragile at scale.
The same set of weight operating on the same input sequences do not produce the same logits between training and inference engines.
This is due to accumulated floating point errors and the different sets of kernels used optimized for training or inference respectively.
...
These effects amplify under clipping and have been observed to trigger irreversible collapse; prior \ac{MoE} work required routing-replay to stabilize \ac{GRPO}.
\cite{gspo2025}
...
With the same group-relative advantage as \ac{GRPO}, \begin{equation}
...
\end{equation} \ac{GSPO} defines a \emph{length-normalized sequence-level} importance ratio \begin{equation}
s_i(\theta)
\;=\;
...
\ac{GSPO}'s sequence weighting eliminates that instability, removes the need for routing-replay in \acs{MoE} \ac{RL}, and tolerates training-versus-inference log-probability precision differences—often allowing direct use of inference-engine likelihoods without recomputation.
...
TauBench~\cite{taubench2024} enables broader assessment of how tool-usage skills transfer to adjacent tasks beyond code repair.
