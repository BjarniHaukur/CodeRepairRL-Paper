...
We begin by situating \ac{APR} historically, then introduce a taxonomy of \ac{LLM}-based approaches—scaffold-free, agentless, and agentic—that clarifies which paradigms support end-to-end policy learning.
With the APR landscape established, we review the instruction-driven datasets that enable training and evaluation of repository-level repair systems.
We then provide an in-depth treatment of policy optimization algorithms, progressing from \ac{PPO} through group-relative methods to \ac{GSPO}, followed by parameter-efficient adaptation via \ac{LoRA}, establishing the theoretical toolkit for the methodology in subsequent chapters.
...
Frontier laboratories subsequently converged on repository-grounded evaluation (SWE-Bench, SWE-Bench-Verified) and deployed first-party agent scaffolds (OpenAI Codex CLI, Anthropic Claude Code, Google Gemini CLI) that orchestrate tool invocations over live repositories~\cite{sweBench2024,sweBenchVerified2024,openaiCodexCLIRepo,claudeCode,geminiCLIRepo}.
...
A representative example is \emph{RepairLLaMA}, which fine-tunes a general code model on curated bug-fix pairs so that it proposes minimal and targeted edits without relying on an execution scaffold~\cite{repairllama2023}.
...
A thin scaffold retrieves relevant files, builds diffs, and structures the prompt; the model returns patch blocks or edit intents; a human or scripted scaffold runs tests and shell commands and applies the edits.
...
\end{verbatim}
\caption{Example JSON tool call structure for an apply\_patch function.}
\label{fig:json-tool-call}
\end{figure}
...
This shift has also shaped how models are packaged and evaluated: recent open-weight releases pair model checkpoints with first-party scaffolds and report agent-style evaluations, while disclosing only partial training details.
We summarize representative examples.

\paragraph{Qwen3\textendash Coder (Alibaba/Qwen team).}
Qwen3\textendash Coder is positioned as an \emph{agentic} coding model and ships with a first-party command-line scaffold (\emph{Qwen Code}) that drives repository-grounded autonomous workflows~\cite{qwen3CoderBlog2025,qwenCodeCLI}.
The release materials emphasize agent behavior in realistic development loops and demonstrate how the paired scaffold surfaces those capabilities.
...
Weights and the scaffold are available, but a reproducible end-to-end training pipeline is not.
...
Instruction-driven datasets provide natural language issue descriptions alongside buggy repository states and ground-truth patches, enabling both training and rigorous evaluation of repository-level repair systems.
Each instance specifies the debugging task through a problem description, enabling agents to learn from how developers communicate bugs rather than from test failure signals alone.

\subsection{Training Datasets}

SWE-Gym~\cite{sweGym2025} provides approximately 2,400 Python bug-fixing tasks extracted from real GitHub repositories, serving as a large-scale training resource for repository-level repair.
SWE-Bench-Multilingual~\cite{sweBenchMultilingual2025} extends instruction-driven debugging beyond Python, offering tasks spanning nine programming languages: Rust, Java, PHP, Ruby, JavaScript, TypeScript, Go, C, and C++.

\subsection{Evaluation Benchmark}

SWE-Bench-Verified~\cite{sweBenchVerified2024} serves as the community evaluation standard, containing approximately 500 carefully validated Python debugging tasks with deterministic test outcomes.
The benchmark measures functional correctness through actual test execution, providing ground-truth signals for whether proposed fixes resolve reported issues without introducing regressions.
...
These properties led to our adoption of \ac{GSPO} as the policy optimization algorithm for the Nano agent training described in \Cref{ch:method}.

\section{\acf{LoRA}} \label{sec:lora}

Parameter-efficient adaptation is essential for training large models without the overhead of full fine-tuning.
\acs{LoRA} learns a low-rank update to frozen weights \cite{lora2021}:
\begin{equation}
