...
To focus optimization on agent-authored content, we employ a dual-masking strategy that carefully separates what the model observes from what drives gradient updates.

\paragraph{Motivation for Selective Loss Masking.}
Interactive agents must process and reason about tool outputs—shell command results, file contents, search results—to maintain coherent multi-turn conversations and make informed decisions.
However, including these externally-generated tokens in the loss computation dilutes the optimization signal.
Consider an agent trajectory $\tau = (s_1, a_1, o_1, s_2, a_2, o_2, \ldots, s_T)$ where $a_t$ represents agent actions such as tool calls and $o_t$ denotes corresponding tool outputs.
While tool outputs exhibit some predictability given sufficient context, training the model to predict them shifts optimization pressure away from the primary learning objective: selecting and invoking appropriate tools based on repository context.
Traditional sequence modeling treats the entire trajectory as a prediction target, thereby dispersing the learning signal across both agent reasoning and external content prediction.

\paragraph{Dual-Mask Strategy.}
We introduce a dual-masking approach that preserves contextual information during forward computation while focusing optimization exclusively on agent-authored content.
...
During the forward pass, the model processes the complete sequence with full causal attention, ensuring that when computing log probabilities for agent-authored tokens, the model maintains access to tool response content through the attention mechanism.
This preserves the contextual information necessary for reasoning about external observations.

For loss computation, we apply selective masking to concentrate optimization on agent decisions.
We define a binary loss mask $\mathbf{M}_{\text{loss}}[i] = 1$ if $i \notin \mathcal{T}$ (agent-authored) and $\mathbf{M}_{\text{loss}}[i] = 0$ if $i \in \mathcal{T}$ (tool-generated).
The masked policy loss then becomes: \[ \mathcal{L}_{\text{policy}} = \frac{\sum_{i=1}^{n} \mathbf{M}_{\text{loss}}[i] \cdot \ell_i \cdot A_i}{\sum_{i=1}^{n} \mathbf{M}_{\text{loss}}[i]} \] where $\ell_i$ represents the per-token policy loss and $A_i$ denotes advantages computed via \ac{GSPO}.

\paragraph{Perplexity Analysis and Training Focus.}
Tool-generated tokens consistently exhibit higher perplexity than agent-authored content, as the model's predicted distributions often diverge from actual tool outputs.
This divergence reflects the inherent difficulty of precisely predicting external system responses without complete environmental state, rather than a deficiency in model capability.
Without selective masking, the optimization objective allocates substantial capacity toward predicting tool outputs—content that while partially predictable represents a distinct learning goal from the desired focus on contextual tool selection and agent reasoning.

\paragraph{Algorithmic Benefits.}
This dual-mask formulation provides several key advantages for training tool-augmented language models.
...
The masking approach integrates seamlessly with existing implementations, avoiding computational overhead.
As tool outputs grow in length and complexity, training remains focused on core agent capabilities rather than becoming diluted across content prediction tasks.
...
\todoinline{Add Appendix references.}
...
Secondary analysis covers patch-similarity rates on SWE-Bench-Verified, per-language reward trends on the SWE-Bench-Multilingual holdout, and harness-efficiency metrics (tool-call success, invalid-call rate, action efficiency).
Tertiary metrics quantify cross-scaffold transfer: we replay checkpoints on Mini-SWE-Agent-, Aider-, and OpenHands-style harnesses to measure zero-shot and few-shot generalization using the protocol shared with the accompanying conference draft.
