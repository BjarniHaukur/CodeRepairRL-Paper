...
\textbf{\ac{GRPO}
Optimization for Agents}: We adapt \ac{GRPO}~\cite{grpo2024} for coding agents, demonstrating its suitability as an alternative to traditional actor-critic methods while reducing computational overhead through elimination of value network training.
...
\textbf{Theoretical Foundation}: Models trained through experiential learning on SWE-Gym should demonstrate generalization to related debugging tasks without severe overfitting.

\textbf{Evidence}: \todoinline{Experimental validation pending: measure cross-dataset performance on SWE-Bench and Multi-SWE-Bench, compare against \ac{SFT} baselines with confidence intervals.}
...
\textbf{Evidence}: \todoinline{Experimental validation pending: evaluate zero-shot transfer to alternative harnesses (Aider/OpenHands-style) and terminal-oriented tasks (\ac{TauBench}/\ac{TerminalBench}).}
