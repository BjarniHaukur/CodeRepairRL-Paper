...
\subsection{RQ3: Execution-Free Multilingual Curriculum Generalization}
\label{subsec:rq3-answer}

\textbf{Theoretical Foundation}: Exposure to the multilingual curriculum should raise average reward on the 50-task SWE-Bench-Multilingual holdout without overfitting to the training languages.
...
\textbf{Significance}: Positive multilingual holdout gains would demonstrate that execution-free rewards enable cross-language transfer without bespoke per-language infrastructure, validating the language-agnostic training approach.
...
\subsection{Enhanced Containerization}
...
Combined with off-policy rollouts described above, this would eliminate one of the primary throughput bottlenecks in the current system.
