...
This thesis investigates online \ac{RL} training for repository-level code repair through a single minimalist agent, demonstrating that execution-free patch-similarity rewards enable meaningful improvements in harness adaptation within academic compute constraints.
...
Our experiments provide evidence addressing the four research questions established in Chapter~\ref{ch:introduction}:

\subsection{RQ0: Training Convergence} \label{subsec:rq0-answer}

\textbf{Research Question}: Does \ac{GSPO} training converge effectively with execution-free rewards?

\textbf{Findings}: Training for approximately 2 epochs demonstrates sustained reward progression throughout, with mean patch-similarity rewards approximately doubling from 0.05 to 0.10.
Crucially, reward variance increases rather than collapsing, maintaining strong policy gradient signal by generating diverse episode outcomes.
Transient declines around step 200 reflect characteristic online learning dynamics where overreinforced patterns degrade performance before superior alternatives emerge.

\textbf{Implications}: Execution-free patch-similarity rewards provide sufficient signal for policy convergence in agentic debugging, validating the core feasibility assumption underlying the entire training approach.
...
\textbf{Findings}: Training substantially improves operational metrics across all measures.
Shell command success rates improve from approximately 45\% to 80\%, while episodes consistently exhaust the 30-tool-call budget early in training but trend downward as the policy learns more efficient debugging workflows.
Command usage patterns shift strategically, with the model developing systematic repository navigation and focused debugging behaviors rather than death-spiral patterns.
...
\textbf{Findings}: Training dramatically improves harness adaptation—patch completion rates more than double from 37\% to 78\% and patch-similarity rewards increase by 54\%—but test-verified success remains flat at approximately 6--7\%.
The baseline reward distribution concentrates heavily near zero (313 empty patches, 150 unresolved with poor patches), creating a hierarchical learning structure where escaping this low-reward regime represents the highest-impact optimization target.
Step 460 demonstrates successful acquisition of operational competence (non-zero reward achievement through reliable tool use) without yet translating to functional correctness improvements.

\textbf{Implications}: The results validate that execution-free patch-similarity rewards enable measurable learning, though the relationship between training rewards and test-verified success proves indirect.
Extended training beyond the current 2+ epochs may be necessary, particularly given that similar methodologies (DeepSWE) achieve strong results with substantially more computational resources and longer training duration.

\subsection{RQ3: Language-Agnostic Training Effectiveness}
\label{subsec:rq3-answer}

\textbf{Research Question}: Does execution-free \ac{RL} improve performance across programming languages without language-specific engineering?

\textbf{Findings}: Training on the 1{,}000-task mixed curriculum (750 Python + 250 multilingual) demonstrates measurable reward improvements across all nine programming languages when comparing epoch 1 to epoch 2 on identical problem instances.
Java, Ruby, and PHP achieve the strongest absolute performance (0.12--0.18), while all languages—from low-level systems languages (C, Rust) to dynamic scripting languages (Python, Ruby, JavaScript) to statically-typed languages (Java, TypeScript)—show clear learning progression without language-specific adaptations.
...
This validates the language-agnostic design principle: by decoupling learning signals from execution infrastructure, a single methodology trains effectively across heterogeneous language ecosystems spanning different paradigms, type systems, and syntactic structures.
...
We mitigate immediate operational risks through restrictive execution environments during training—restricted bash, per-episode tool-call limits, and wall-clock timeouts—though we acknowledge these measures target training safety rather than preventing determined misuse of released models.
...
While \ac{GSPO} training substantially improved harness adaptation—more than doubling patch completion rates from 37\% to 78\%—test-verified success on SWE-Bench-Verified remained flat at approximately 6-7\%.
...
This work demonstrates that online \ac{RL} training for coding agents is practically feasible within academic compute constraints and yields measurable improvements in harness adaptation.
