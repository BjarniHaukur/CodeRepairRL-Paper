...
% Detail/Nuance
We implement this through \ac{GSPO}~\cite{gspo2025} with an explicit KL regularizer, enabling stable updates while models actively debug real repositories during training.\todoinline{Verify GSPO citation details once bibliography is finalized.}
...
Our \textit{Nano-agent} architecture provides essential bash and file operations, demonstrating that sophisticated debugging behaviors can emerge from simple interfaces when combined with \ac{RL}.\todoinline{Add brief description of Mini-SWE-Agent/Aider transfer once experiments finish.}

% Evidence / Secondary Contribution
Experiments on [X training instances, Y compute hours] demonstrate that online training yields models exhibiting sophisticated debugging behaviors: systematic hypothesis testing, incremental code modification, and test-guided exploration [specific examples needed].\todoinline{Insert zero-shot transfer summary once scaffold experiments are finalized.}
On SWE-Bench-Verified, our approach achieves [X\% resolve rate], representing [Y\% improvement over baselines].
Secondary findings include emergent systematic debugging strategies and significant computational efficiency gains through training-inference duality.\todoinline{Insert concrete efficiency numbers once evaluation completes.}
...
\subsection*{Keywords}
automated program repair, coding agents, reinforcement learning, execution-free training, repository-level evaluation, reproducibility, multilingual
\todoinline{populate}
...
These sessions provided an invaluable venue for discussing ideas and hearing what others working on similar research are thinking.
