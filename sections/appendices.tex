...
Qwen3-14B offered an optimal fit for available resources: Qwen3-8B proved too large for two \acp{GPU} yet too small to justify three, while the 32B variant though promising, imposed prohibitive computational demands for exploratory training runs.
...
These constraints narrowed the viable candidates to the Qwen3 family, where robust tool-calling support, strong baseline coding performance, and flexible scaling across 8B, 14B, and 32B enabled systematic capacity analysis within consistent infrastructure.

\Cref{fig:model-reward-comparison,fig:model-tool-comparison} compare training trajectories across model families.
Llama3.1-8B plateaus near zero reward with minimal tool success, motivating its exclusion from main results.
Qwen3-32B reaches 0.20 reward before early termination at step 135 due to compute constraints.
Qwen3-14B and 8B exhibit comparable performance trajectories.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{plotting/figures/plots/comparison/reward_comparison_win20_multi.png}
	\caption{Mean reward progression across model families and sizes, plotted with rolling window average.}
	\label{fig:model-reward-comparison}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{plotting/figures/plots/comparison/tool_success_comparison_win20_multi.png}
	\caption{Tool success rates across models. Left: shell commands. Right: apply\_patch.}
	\label{fig:model-tool-comparison}
\end{figure}
...
8 & v3.2.0 & gpt-4.1-mini & \texttt{0.350} & \texttt{0.017} & \texttt{7,403 / 16,384} & \texttt{29.7 / 100} \\
9 & v3.2.0 & deepseek-chat & \texttt{0.336} & \texttt{0.011} & \texttt{3,297 / 16,384} & \texttt{7.5 / 100} \\
10 & v4.0.1 & glm-4.5 & \texttt{0.323} & \texttt{0.107} & \texttt{12,477 / 16,384} & \texttt{28.7 / 100} \\
11 & v3.2.0 & qwen-2.5-72b-instruct & \texttt{0.272} & \texttt{0.000} & \texttt{5,873 / 16,384} & \texttt{35.1 / 100} \\
12 & v3.2.0 & qwen3-32b & \texttt{0.255} & \texttt{0.000} & \texttt{5,281 / 16,384} & \texttt{28.3 / 100} \\
13 & v3.2.0 & llama-4-maverick & \texttt{0.255} & \texttt{0.000} & \texttt{4,647 / 16,384} & \texttt{10.4 / 100} \\
14 & v3.2.0 & qwen3-8b & \texttt{0.190} & \texttt{0.000} & \texttt{8,704 / 16,384} & \texttt{56.5 / 100} \\
15 & v3.2.0 & gpt-4.1-nano & \texttt{0.188} & \texttt{0.000} & \texttt{8,536 / 16,384} & \texttt{33.1 / 100} \\
16 & v3.2.0 & qwen3-14b & \texttt{0.176} & \texttt{0.000} & \texttt{10,800 / 16,384} & \texttt{82.6 / 100} \\
17 & v3.2.0 & devstral-small & \texttt{0.092} & \texttt{0.000} & \texttt{14,603 / 16,384} & \texttt{13.0 / 100} \\
\bottomrule
\end{tabular}
\end{table}
...
The main repository at \url{https://github.com/ASSERT-KTH/CodeRepairRL} contains complete training scripts, SLURM job templates, and episode orchestration logic.
The thesis repository at \url{https://github.com/BjarniHaukur/CodeRepairRL-Paper} contains the \LaTeX{} source for this document alongside all analysis and plotting scripts, which use the Weights \& Biases API to access logged experimental data to generate figures.
