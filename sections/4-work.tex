...
Recent \ac{RL} training libraries have begun supporting out-of-process generation, where rollout collection runs on separate inference servers rather than within the trainer process itself.
...
\section{\acs{SLURM}
  Orchestration Workflow} \label{sec:slurm-orchestration}

All experiments run on the Berzelius and Alvis clusters provided by \ac{NAISS}, which deploy \ac{SLURM} as their workload manager for allocating compute resources and scheduling batch jobs.
The complete training system executes within a single SLURM batch job that allocates three A100 \acp{GPU}—two for distributed training and one for inference serving—and partitions them between processes using \acs{CUDA} device visibility controls.
The job script spawns the vLLM server on the inference \ac{GPU}, then launches the DeepSpeed-coordinated trainer on the training \acp{GPU}, with all processes sharing the same SLURM allocation and communicating via the cluster's InfiniBand interconnect.
Dependency management employs Apptainer/Singularity containers, necessary for installing the complete software stack with exact library versions on shared cluster environments where users lack root privileges.
Complete SLURM submission scripts, environment configurations, and all implementation code are released open-source; details appear in \cref{app:reproducibility}
