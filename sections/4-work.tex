\chapter{Training Infrastructure}
\label{ch:work}

Chapter~\ref{ch:method} established the methodology: execution-free patch-similarity rewards, the Nano agent specification, \ac{GSPO} training, and evaluation protocol.
...
We detail the training environment implementation, including live adapter synchronization via \ac{NCCL}, trainer extensions for masked loss and variable-turn episodes, and orchestration under SLURM.
...
The inference system builds on vLLM's asynchronous engine, leveraging its continuous batching and OpenAI-compatible \acs{API} to execute multi-turn episodes.
We extended vLLM's async engine with \ac{NCCL}-based live adapter synchronization, allowing policy improvements to propagate to the running inference server without interruption.
...
The Nano agent incorporates explicit resource constraints, recognizing that multi-turn agent interactions impose substantially higher computational demands than many traditional training objectives.
Episode limits serve two practical purposes: bounding \ac{VRAM} utilization and improving sample efficiency.
...
While we could guard against training \ac{GPU} OOM through aggressive truncation, this risks removing important tokens from the rollouts that contributed to the reward the agent got.
Instead, we set token budgets that accommodate debugging workflows while remaining within memory constraints.
...
Importantly, the agent receives warnings as it approaches these limits, encouraging it to produce a submission attempt rather than simply timing out without any learning signal.
This improves sample efficiency by ensuring episodes yield gradient information even when the agent has not fully solved the task.
...
Doing KL with LoRA is a free win since we can just unmount the adapter to get the original policy.
We do not need 2x the params.

\subsection{Triton kernel implementation of the \acs{GSPO} kernel}
\todoinline{write}
