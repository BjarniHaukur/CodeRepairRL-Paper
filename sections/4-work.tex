...
This chapter describes the comprehensive implementation effort required to realize online \ac{RL} for automated code repair.
...
Implementing online \ac{RL} for code repair presented several unique engineering challenges not addressed by existing frameworks.
Traditional \ac{RL} libraries assume simple observation-action spaces and stateless environments, while our system requires:
...
Our implementation comprises several interconnected subsystems, each addressing specific aspects of the online RL paradigm:

\subsubsection{Agent Framework Integration}

\textbf{OpenAI \ac{API} Compatibility}: The Nano agent originally designed for commercial \ac{LLM} \acp{API} was adapted to work with our vLLM serving infrastructure through a compatibility layer that translates between \ac{API} formats while preserving all agent capabilities.

\textbf{Trajectory Serialization}: Agent interactions produce complex nested data structures (commands, outputs, file states, error messages) that must be serialized into formats suitable for \ac{RL} training.
Our serialization protocol preserves all interaction details while maintaining computational efficiency.

\textbf{Action Space Mapping}: The agent's tool calls are mapped to discrete actions in the \ac{RL} formulation, with careful attention to maintaining gradient flow through the policy network to the underlying token generation process.
...
\textbf{Resource Management}: Containers are allocated specific \ac{CPU}, memory, and storage quotas to ensure fair resource distribution and prevent resource exhaustion attacks from malformed agent behavior.
...
While existing implementations of \ac{GRPO}~\cite{shao2024deepseekmathpushinglimitsmathematical} existed in the TRL library \cite{trl2020}, they were not designed for the complex interaction patterns required by coding agents.
Our custom implementation addresses several limitations:

\textbf{Variable-Length Trajectories}: Standard \ac{GRPO} assumes fixed-length sequences, while agent interactions vary dramatically in length depending on problem complexity and exploration strategy.
...
\textbf{Multi-\ac{GPU} Training}: Model training utilizes DeepSpeed ZeRO-3 for efficient parameter and gradient sharding across multiple \acp{GPU}, with careful attention to load balancing given the variable computational costs of different agent interactions.
...
Adapting vLLM for \ac{RL} training required several modifications to the base serving infrastructure:
...
This required careful coordination of \ac{CUDA} memory management, request routing, and state consistency.

\textbf{Multi-Model Serving}: \ac{GRPO} requires serving both the current policy and a reference policy simultaneously.
Our implementation shares base model weights while maintaining separate \ac{LoRA} adapters, optimizing memory usage and inference throughput.
...
\textbf{Batch Processing Optimizations}: Custom batching algorithms pack variable-length agent requests to maximize \ac{GPU} utilization while respecting memory constraints and conversation coherence requirements.
...
The real-time weight synchronization system represents one of the most technically challenging aspects of the implementation, requiring deep integration with \ac{CUDA} programming and distributed systems concepts.

\subsection{Technical Architecture}

\subsubsection{NCCL Integration Design}

Our \ac{NCCL}-based communication system operates through several layers:
...
\textbf{Memory Management}: Sophisticated \ac{CUDA} memory management coordinates between training and inference processes, utilizing unified memory and peer-to-peer access where available.
...
\textbf{Asynchronous Operations}: Weight broadcasts occur asynchronously with respect to inference requests, utilizing \ac{CUDA} streams and events to prevent blocking of ongoing inference work.
...
\subsubsection{\ac{CUDA}
	Memory Coordination}

Coordinating \ac{CUDA} memory across multiple processes presented several unique challenges:

\textbf{Memory Fragmentation}: Long-running training jobs tend to fragment \ac{GPU} memory, leading to allocation failures.
We implemented custom memory pools and defragmentation strategies to maintain stable operation.

\textbf{Inter-Process Communication}: Efficient weight sharing required implementing custom IPC mechanisms using \ac{CUDA} unified memory and peer-to-peer access, with fallbacks to \ac{CPU} memory for compatibility.
...
Adapting \ac{SWE-Gym} for \ac{RL} training required significant data processing and infrastructure work:

\subsubsection{Container Environment Setup}

Each \ac{SWE-Gym} task required careful containerization to ensure reproducible and safe execution:
...
Converting \ac{SWE-Gym} tasks into \ac{RL} training episodes required sophisticated data processing:
...
Preparing \ac{SWE-Bench-Verified} for evaluation required several processing steps:
...
A key objective of this work was providing open-source implementations to democratize access to online \ac{RL} techniques.
...
The implementation process revealed several important insights about the practical challenges of online RL:
...
\textbf{Debugging Difficulty}: Debugging distributed agent systems is substantially more complex than traditional \ac{ML} training, requiring sophisticated logging and monitoring infrastructure to identify root causes of failures.
...
These insights provide valuable guidance for future research in experiential training and highlight the substantial engineering effort required to make theoretical advances practical and reproducible.
