...
\paragraph{The Async Multi-Turn Challenge.}
...
\paragraph{Weight Synchronization Architecture.}
...
This memory-bounded gathering strategy enables weight synchronization to coexist with ongoing inference and training operations without triggering out-of-memory failures, even on memory-constrained accelerators.
...
\paragraph{Performance and Generality.}
...
\paragraph{Broader Applicability Beyond Coding Agents.}
...
The trainer integrates masked loss (see Appendix~\ref{app:masked-loss}), sequence-level importance ratios computed after masking, and variable-turn rollout collection that preserves turn boundaries while remaining batch-friendly.
...
We achieved this through systematic application of memory and computational optimizations that, when composed carefully, enable training of models up to 30B parameters on modest \ac{GPU} clusters.

\paragraph{Parameter-Efficient Adaptation with \ac{LoRA}.}
...
\paragraph{Memory Management Through DeepSpeed ZeRO.}
...
We employ DeepSpeed ZeRO-2 partitioning for models up to 14B parameters and ZeRO-3 for larger configurations.
...
This configuration suffices for 8B and 14B models where parameter memory remains manageable but optimizer states—particularly for AdamW with its first and second moment estimates—would otherwise exceed available \ac{VRAM}.
For the 30B-class models, ZeRO-3 extends sharding to include model parameters themselves, gathering required parameters on demand during forward and backward passes while keeping per-device memory consumption bounded.

\paragraph{Gradient Checkpointing for Activation Memory.}
...
\paragraph{Mixed-Precision Training with BF16.}
...
\paragraph{Custom Triton Kernels for \ac{GSPO}
Loss Computation.
}
...
\paragraph{Kernel Optimizations for Attention and Serving.}
...
\paragraph{Resource Requirements and Scalability.}
The composed optimizations yield manageable resource requirements across model scales.
Qwen3-8B trains comfortably on 2-3 A100 \acp{GPU} with ZeRO-2 and \ac{LoRA}, Qwen3-14B on 3-4 \acp{GPU} with the same configuration, and Qwen3-30B-A3B on 6-8 \acp{GPU} with ZeRO-3.
Inference serving requires 1-2 \acp{GPU} depending on model size and desired throughput.
These requirements remain within the reach of academic research groups, demonstrating that sophisticated online \ac{RL} training need not demand industrial-scale compute infrastructure when careful optimization and architectural choices reduce overhead systematically.
...
The composed infrastructure scales across model sizes while maintaining consistent training dynamics.
Qwen3-8B training proceeds efficiently on 2-3 A100 \acp{GPU}, Qwen3-14B on 3-4 \acp{GPU}, and Qwen3-30B-A3B on 6-8 \acp{GPU} as detailed in \S4.6.
\ac{GPU} utilization remains high throughout training through continuous batching and overlapped computation-communication patterns.
\todoinline{Document \acp{GPU} scaling linearity, memory utilization efficiency, fault tolerance characteristics, and performance bottlenecks identified during extended training runs.}

\subsection{Cost-Effectiveness for Academic Research}

Total training costs remain accessible for academic research groups.
\todoinline{Insert cost analysis table with total A100 \ac{GPU}-hours per run for each model size, per-episode cost comparisons, development cost implications, and infrastructure maintenance requirements.}
...
We release the Nano tool schema, system prompts, trainer configurations, \ac{LoRA} hyperparameters, data selection and deduplication scripts, pinned package manifests, and SLURM submission templates to enable full replication of our training setup.
Per-episode logs capture comprehensive execution traces including tool invocations (with truncation markers), token counts per turn, wall-clock timings, canonical diffs, computed rewards, and RNG seeds, providing complete transparency into the training process.
...
Figures include an agent loop diagram, a concise system architecture, and an annotated rollout example.
