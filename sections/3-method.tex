...
\textbf{Late-Layer Focus:} We apply LoRA exclusively to the last third of transformer layers (e.g., layers 42-63 for 32B models), preserving early linguistic representations that encode fundamental language understanding.

\textbf{Low-Rank Constraint:} Using rank $r \leq 8$ with $\alpha = 2r$ limits the parameter update magnitude to approximately 1\% of backbone weights, preventing drift while enabling task adaptation.

\textbf{Two-Stage Adaptation:} We employ separate LoRA adapters for SFT and RL phases, enabling fine-grained control over each training stage and preventing interference between supervised and reinforcement learning objectives.

These techniques, detailed comprehensively in Section~\ref{sec:distillation-pipeline}, form the foundation of our stable training pipeline that achieves strong task performance without sacrificing general capabilities.
...
\section{Distillation Challenges and Two-Stage Training Pipeline}
...
\subsection{Initial Distillation Attempts: Catastrophic Forgetting}

Our first approach attempted standard supervised fine-tuning (SFT) on synthetic agent trajectories, using full-rank LoRA across all transformer layers. This seemingly straightforward approach produced devastating results, summarized in Table~\ref{tab:distillation-failures}.

\begin{table}[ht]
\centering
\caption{Observations during initial SFT/LoRA training and their downstream impacts}
\label{tab:distillation-failures}
\begin{tabular}{p{5cm}|p{4cm}|p{5cm}}
\toprule
\textbf{Training Observation} & \textbf{Downstream Symptom} & \textbf{Root Cause} \\
\midrule
Token-level loss $\downarrow$, per-token accuracy $\uparrow$ & Pass-rate on hold-out SWE-Gym drops by double digits & Catastrophic forgetting: narrow, teacher-forced corpus overwrote circuits handling general reasoning~\cite{allroads2025} \\
\hline
...
The key insight from these failures: the student policy is evaluated in \textit{free-run} mode during deployment, but was only trained to \textit{mimic} teacher tokens. This mismatch, combined with limited model capacity, causes large drift despite small apparent token-level loss.

\subsection{Minimal, High-Leverage Fixes}

Based on these observations, we developed three targeted interventions that dramatically improve distillation outcomes:

\subsubsection{Low-Rank, Late-Layer LoRA}

Rather than updating all parameters, we constrain adaptation to:
\begin{itemize}
    \item \textbf{Rank $\leq 8$}: Minimal parameter updates
    \item \textbf{Layers = last $\frac{1}{3}$ of transformer}: Preserve early linguistic representations
    \item For Qwen2.5-Coder-7B: layers 22-31
    \item For Qwen2.5-Coder-32B: layers 42-63
\end{itemize}
...
\subsubsection{KL-Anchored SFT}

We augment the standard cross-entropy loss with a KL divergence term:
\begin{equation}
\mathcal{L}_{\text{SFT-KL}} = \mathcal{L}_{\text{CE}} + \lambda \cdot \text{KL}(\pi_{\text{student}} \| \pi_{\text{frozen-base}})
\end{equation}
...
\subsection{Two-Stage Training Pipeline v2.0}

These insights led to our refined two-stage pipeline, illustrated in Figure~\ref{fig:training-pipeline-v2}:

\begin{figure}[ht]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering 
        \textit{[Figure placeholder: Flow diagram showing:
        1. Pretrained Qwen2.5-Coder
        2. SFT LoRA (r=8, late blocks) with KL anchor + replay
        3. Freeze SFT adapter
        4. Add second LoRA (r=8, late blocks) for GRPO
        5. GRPO on-policy RL with reward = SWE-Gym bug-fix score
        6. Merge adapters → final inference model]}
    }}
    \caption{Two-stage training pipeline with separate adapters for SFT and RL phases, enabling stable task adaptation while preserving general capabilities.}
    \label{fig:training-pipeline-v2}
\end{figure}

\textbf{Stage 1: Supervised Fine-Tuning}
\begin{itemize}
    \item Apply LoRA (r=8, $\alpha$=16) to last third of layers
    \item Train with $\mathcal{L}_{\text{CE}} + 0.05 \cdot \text{KL}$
    \item Include 20\% replay data per epoch
    \item Result: SFT checkpoint with task knowledge but preserved general capabilities
\end{itemize}

\textbf{Stage 2: Reinforcement Learning}
\begin{itemize}
    \item Freeze SFT adapter
    \item Add second LoRA adapter with identical configuration
    \item Run GRPO with KL penalty to SFT checkpoint (not base)
    \item KL budget: start at 0.03, anneal when reward plateaus
    \item Monitor: reward $\uparrow$, KL(new$\|$SFT) $< 0.3$, held-out QA loss drop $< 3$ pp
\end{itemize}

\subsection{Empirical Results}

Table~\ref{tab:pipeline-results} summarizes the improvements from our refined pipeline:

\begin{table}[ht]
\centering
\caption{Performance comparison across training stages (2024-25 ablations)}
\label{tab:pipeline-results}
\begin{tabular}{l|l|c|c}
\toprule
\textbf{Stage} & \textbf{Configuration} & \textbf{$\Delta$ Pass-Rate (SWE-Gym)} & \textbf{$\Delta$ General QA} \\
\midrule
Base $\rightarrow$ SFT (v1) & Full-rank LoRA, all layers & -15 pp & -4 pp \\
Base $\rightarrow$ SFT (v2) & \textbf{r=8, late; KL + replay} & \textbf{+6 pp} & -1 pp \\
SFT (v2) $\rightarrow$ RL (GRPO) & New r=8, late adapter & \textbf{+10 pp} & -3 pp \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{LoRA Configuration (Hydra/YAML):}
\begin{verbatim}
lora_config:
  r: 8
  lora_alpha: 16                    # ~2×r
...
\textbf{Training Modifications:}
\begin{itemize}
    \item Replace standard \texttt{SFTTrainer} loss with CE + KL implementation
    \item Maintain frozen base model for KL computation
    \item Implement data mixing: 80\% task data, 20\% replay per batch
    \item Use separate optimizers for each LoRA adapter stage
\end{itemize}

\textbf{Monitoring Metrics:}
\begin{itemize}
    \item Primary: SWE-Gym pass rate on held-out bugs
    \item Secondary: KL divergence from base/SFT checkpoints
    \item Sanity check: Performance on HumanEval and MBPP benchmarks
    \item Memory: Peak GPU utilization should remain stable across stages
\end{itemize}

\subsection{Key Takeaways}

The initial "lobotomy" effect arose from capacity-limited, teacher-forced SFT that overwrote core linguistic blocks. By constraining parameter updates (low-rank, late-layer), anchoring to the base distribution with KL regularization, and maintaining the original distribution through replay, we achieve stable task adaptation without catastrophic forgetting. These low-overhead modifications fix the fundamental training issues \textit{before} investing substantial compute in on-policy RL, and remain essential biases throughout the full training pipeline.
