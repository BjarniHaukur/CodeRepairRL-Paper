...
\begin{figure}[htbp]
    ...
    \caption{Sequence of interactions in our online training loop for the Nano agent. The depicted configuration uses single-step gradient accumulation (i.e., updates are computed per collected batch without additional accumulation). Each agent episode can expand to an arbitrary number of API/tool calls, and request–response handling is fully asynchronous: agent requests are issued without blocking, and completions stream back independently, allowing episodes of different lengths to proceed concurrently.}
