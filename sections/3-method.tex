...
J(\theta) = J_{\mathrm{GSPO}}(\theta) - \beta_{\text{KL}} \cdot \mathbb{E}_{x \sim \mathcal{D}} \left[ D_{\text{KL}}\!\big(\pi_\theta(\cdot \mid x) \,\|\, \pi_{\theta_{\text{ref}}}(\cdot \mid x)\big) \right],
\end{equation} where $J_{\mathrm{GSPO}}(\theta)$ denotes the sequence-level clipped surrogate objective detailed in \S\ref{subsubsec:gspo}, $\beta_{\text{KL}}=0.01$ controls regularization strength, and $\pi_{\theta_{\text{ref}}}$ is the initial pre-trained checkpoint frozen at training start.
...
\todoinline{this information should be moved elsewhere / to appendix}
Unless noted otherwise, we use group size $G=8$ responses per prompt, temperature 1.0 (no top-p/top-k sampling), and importance ratio clipping $\varepsilon=0.2$.
...
\textbf{RQ3 (Language-Agnostic Training Effectiveness):}
We evaluate whether execution-free \ac{RL} enables effective multilingual training without language-specific engineering by measuring per-language reward trends on the 50-task SWE-Bench-Multilingual held-out set with bootstrap confidence intervals, comparing pre-training and post-training checkpoints across nine programming languages.
