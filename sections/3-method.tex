...
\end{minted}
\caption{Illustrative episode execution of the Nano agent. Strings in red are supplied by the \ac{LLM} as \ac{JSON} tool call arguments. The agent explores repository structure and file contents before applying a targeted edit.}
...
During training episodes, we configure sampling with temperature 1.0 and top-$p$ 1.0 to encourage exploration of diverse debugging strategies and tool-usage patterns.
...
\end{equation} where the \ac{KL} divergence is computed on the logits of the episodes used for training, $\beta_{\text{KL}}$ controls regularization strength, and $\pi_{\theta_{\text{ref}}}$ is the initial pre-trained checkpoint.
