...
Rather than constraining models to single-pass generation, we enable multi-step interactions where agents can: \begin{itemize}
...
This format is naturally conducive to language model generation because: \begin{itemize}
...
The Nano agent instantiates the history-based MDP formulation from \cref{subsec:mdp-history} as follows:
...
Each task includes: \begin{itemize}
...
Qwen models exhibit exceptional structured output generation, critical for reliable agent behavior: \begin{itemize}
...
Beyond tool calling, Qwen demonstrates strong foundational coding abilities: \begin{itemize}
...
Can also "mine" the trainer/config.py for relevant stuff to add}
...
\textbf{vLLM Inference Cluster:} 1 $80$GB A100 for $8$B/$14$B models, 2 for $~32$B \textbf{TRL Training Cluster:} 2 $80$GB A100 for $8$B/$14$B models, 4 for $~32$B, launches agents, posts weight synchronizations
