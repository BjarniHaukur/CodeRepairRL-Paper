...
% Removed placeholder figure that previously pointed to plotting/figures/diagram.png
...
% We retain the same outcome-driven similarity principle as SWE-\ac{RL}~\cite{wei2025swerladvancingllmreasoning} while embedding it within a multi-step setting that rewards successful end-to-end repair after autonomous context acquisition.
...
% Our Nano coding agent can therefore engage in natural back-and-forth interaction with repositories through basic terminal commands while remaining fully compatible with the \ac{RL} training loop.
...
To ground the interface in practice, Figure~\ref{fig:nano-rollout} shows a representative Nano episode captured from the command-line interface during training.
The rollout illustrates repository exploration via shell commands, targeted file inspection, and a single, atomic patch application.
Episode length adapts to task complexity, and request-response handling remains asynchronous throughout, so multiple episodes can progress concurrently without blocking.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{plotting/figures/nano_cli.png}
    \caption{Illustrative rollout of the Nano agent during training. The agent performs repository navigation (e.g., directory listings and ripgrep searches), inspects candidate files, and submits a minimal patch when sufficient evidence accumulates. Interactions are served asynchronously and episode length is adaptive, matching the multi-turn nature of repository-level repair.}
...
\subsection{Integration with \ac{RL} Training}
...
\todoinline{Consider adding: Partial observability challenge - repositories have thousands of files but \ac{LLM} context windows are limited.
...
% This makes \ac{MoE} attractive for multi-turn, long-context training and evaluation.
...
\todoinline{rework, integrate \ac{MoE}, the 2-3 different models we are targeting etc.}
...
\subsection{Masked Loss Computation for Tool-Augmented \ac{RL}} \label{sec:masked-loss-computation}
...
Finally, policy updates modify model parameters to increase the likelihood of high-advantage actions while maintaining proximity to the reference policy through \ac{KL} regularization.
...
Stability is ensured through a \ac{KL} coefficient of $\beta = 0.2$, as omitting \ac{KL} divergence constraints often leads to model collapse in our experiments.
...
Sparse reward handling through \ac{GRPO}'s group-relative advantages.
...
\textbf{\ac{LoRA}:}
\todoinline{Move discussions on what \ac{LoRA} buys us efficiency-wise here, discuss them tersely}

\paragraph{Note on \ac{LoRA} in \ac{KL}-controlled training.}
When \ac{KL} control to a reference policy is used, sharing the frozen $W_0$ and differing only in adapters avoids duplicating full parameters and simplifies bookkeeping; we do not employ an explicit \ac{KL} term in this work but note the general advantage.
...
\section{Training-Inference Duality in Online \ac{RL} for LLMs}
...
Figure~\ref{fig:training-sequence-diagram} summarizes the end-to-end dataflow for the Nano agent during online training, highlighting how variable-length agent episodes interface with asynchronous serving and continuous optimization.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{plotting/figures/training_sequence_diagram.png}
    \caption{Sequence of interactions in our online training loop for the Nano agent. The depicted configuration uses single-step gradient accumulation (i.e., updates are computed per collected batch without additional accumulation). Each agent episode can expand to an arbitrary number of \ac{API}/tool calls, and request–response handling is fully asynchronous: agent requests are issued without blocking, and completions stream back independently, allowing episodes of different lengths to proceed concurrently.}
...
Reward sparsity challenge - most trajectories fail, \ac{GRPO} extracts learning signals through group-relative advantages.
}

\subsection{Outcome-Based Patch Similarity}

Our reward function, inspired by the method proposed in SWE-\ac{RL}~\cite{wei2025swerladvancingllmreasoning}, employs patch similarity as the primary signal for training.
...
The sparse nature of exact patch matching presents challenges for \ac{RL} training.
...
\todoinline{Consider adding: The dual computation problem - online \ac{RL} for \acp{LLM} requires maintaining two distinct workloads: inference (low latency, high throughput, KV caching) and training (gradient accumulation, optimizer states, memory checkpointing).
}

\section{vLLM}

\todoinline{Write short explanation of vLLM choice: why vLLM over SGLang, key features that matter for online \ac{RL} (batching, memory optimization), extensibility for our \ac{NCCL} modifications}
...
The system implements differential updates through sequential parameter gathering, which selectively broadcasts only modified parameters (specifically, the \ac{LoRA} adapter weights) rather than entire model states.
...
Weight broadcasts complete in 150-300ms for \ac{LoRA} adapters with ranks between 8 and 64, enabling near real-time policy improvements throughout the training process.
...
This infrastructure represents a significant advance in making online \ac{RL} practical for large language models, reducing the traditional barrier between training and deployment while maintaining production-grade reliability and performance.
...
The TRL training cluster utilizes two 80GB A100 \acp{GPU} for smaller models and four \acp{GPU} for 32B parameter models, managing both agent trajectory generation and weight synchronization across the distributed system.

\subsection{Implementation Details}

Several key implementation choices enable efficient and safe operation of the distributed system.
Collective RPC facilitates efficient weight sharing across distributed processes, while \ac{LoRA} adaptation provides optional use of low-rank adapters to reduce communication overhead.
...
Figure~\ref{fig:system-architecture} presents the complete system architecture, illustrating data flow between the vLLM inference cluster, distributed agent worker pool, and \ac{GRPO} training pipeline.
The diagram highlights the \ac{NCCL} communication layer that enables real-time weight synchronization, supporting the training-inference duality that distinguishes our approach from conventional \ac{RL} systems.
...
Our primary comparison focuses on the performance delta achieved through our \ac{RL} training approach, measured against the base Qwen models without any task-specific fine-tuning.
