...
To ground the interface in practice, Figure~\ref{fig:nano-rollout} shows a representative Nano episode captured from the command-line interface during training. The rollout illustrates repository exploration via shell commands, targeted file inspection, and a single, atomic patch application. Episode length adapts to task complexity, and request–response handling remains asynchronous throughout, so multiple episodes can progress concurrently without blocking.
