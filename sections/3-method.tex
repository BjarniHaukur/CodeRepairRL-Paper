...
\textbf{Generalization Testing:} To assess transfer of learned repair skills beyond the training domain, we evaluate on HumanEval, a standard code generation benchmark. This tests whether debugging strategies learned through "tool-mediated" RL improve general programming capabilities.
...
Our "tool-mediated" system eliminates this separation. The GRPO process operates as a continuous cycle where:
\begin{enumerate}
...
We employ Group Relative Policy Optimization (GRPO) as our primary RL algorithm. GRPO offers several advantages for our "tool-mediated" setting:
...
Training and serving 8B to 32B parameter models for "tool-mediated" RL requires sophisticated compute optimization across the entire pipeline.
...
These innovations make large-scale "tool-mediated" RL practically feasible for academic research, democratizing access to advanced coding agent training previously available only to industry labs with unlimited compute budgets.
...
The complete "tool-mediated" RL system consists of several interconnected components operating in parallel:
...
This comprehensive evaluation framework ensures that our conclusions about "tool-mediated" RL are well-supported and reproducible.
