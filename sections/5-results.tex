...
Our \ac{GRPO}~\cite{grpo2024} training demonstrates clear and consistent learning dynamics across multiple experimental runs.
...
\todoinline{Analyze \ac{GRPO} training stability compared to traditional \ac{PPO} implementations.
...
\end{itemize}
Provide evidence for substantial and statistically significant \ac{RL} improvements.}
...
\section{RQ3: SWE-Gym
  Training vs.
  SWE-Bench/Multi-SWE-Bench
  Evaluation} \label{sec:rq3-overfit}

\textbf{Research Question 3}: Does training on SWE-Gym lead to overfitting, or do learned debugging strategies generalize to unseen evaluation datasets?

\subsection{Experimental Design}
\label{subsec:rq3-design}
We assess generalization by training exclusively on SWE-Gym and evaluating on held-out datasets: \begin{itemize}
	\item Training dataset: SWE-Gym repository instances
	\item Evaluation datasets: SWE-Bench and Multi-SWE-Bench
...
\item \ac{GRPO} vs. traditional \ac{PPO} comparison
...
\item \ac{GRPO} improvements over traditional \ac{PPO} methods
...
\textbf{RQ1 (Harness Adaptation)}: \todoinline{Summarize \ac{RL} training improvements in Nano harness adaptation, including concrete metrics for tool success rates, invalid call reductions, and action efficiency improvements.
