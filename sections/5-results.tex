...
Our GRPO training demonstrates clear and consistent learning dynamics across multiple experimental runs. Figure~\ref{fig:training-curves} shows the evolution of reward and success rate throughout training for both 8B and 32B parameter Qwen models using the nano-agent scaffold.
...
\section{Main Results: Agent-in-the-Loop vs. Baselines}
\label{sec:main-results}

\subsection{SWE-Bench-Verified Performance}

Table~\ref{tab:main-results} presents our primary experimental results on SWE-Bench-Verified, comparing agent-in-the-loop RL training against multiple baseline approaches.
...
\section{Scaffold Complexity Analysis}
\label{sec:scaffold-analysis}

\subsection{Minimalist vs. Feature-Rich Comparison}
...
\begin{table}[h]
\centering
\caption{Scaffold complexity comparison on SWE-Bench-Verified subset (200 tasks)}
\label{tab:scaffold-comparison}
\begin{tabular}{lcccc}
\toprule
\textbf{Scaffold Type} & \textbf{Success Rate} & \textbf{Avg. Steps} & \textbf{Training Time} & \textbf{Memory Usage} \\
\midrule
Nano-Agent (Minimal) & \textbf{28.5\%} & 12.3 & 1.0x & 1.0x \\
Aider-Based (Rich) & 26.8\% & 8.7 & 1.8x & 2.3x \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Performance Parity Despite Complexity Difference}
...
\begin{itemize}
\item \textbf{Learning Overcomes Engineering}: Given sufficient training, models can develop sophisticated behaviors using simple tools rather than requiring pre-engineered solutions
\item \textbf{Generalization Through Simplicity}: Minimal assumptions about problem structure enable broader learning across diverse bug types and repository structures
\item \textbf{Computational Efficiency}: Simple scaffolds enable more extensive training within the same computational budget, potentially leading to better ultimate performance
\end{itemize}

\subsubsection{Training Efficiency Advantages}

The nano-agent demonstrates significant training efficiency advantages:

\begin{itemize}
\item \textbf{45\% faster training}: Simpler action spaces and fewer tool interactions enable faster episode completion
\item \textbf{57\% lower memory usage}: Minimal tool state and context management reduce memory overhead
\item \textbf{Better sample efficiency}: Clear action-outcome relationships facilitate more effective learning from experience
\end{itemize}
...
\textbf{Nano-Agent Behaviors}:
\begin{itemize}
\item Develops systematic file exploration strategies using grep and find commands
\item Learns to piece together understanding from multiple partial observations
\item Shows creative use of basic tools for complex analysis tasks
\item Demonstrates robust error recovery when simple approaches fail
\end{itemize}

\textbf{Aider-Based Behaviors}:
\begin{itemize}
\item Relies heavily on provided repository analysis and semantic search
\item Shows faster initial convergence but potentially less robust generalization
\item Develops dependence on engineered features that may not transfer to new contexts
\item Exhibits more deterministic, less exploratory problem-solving patterns
\end{itemize}

\section{Generalization and Transfer Learning}
\label{sec:generalization}

\subsection{Cross-Language Transfer: Java Evaluation}
...
\begin{table}[h]
\centering
\caption{Cross-language generalization results on Defects4J v2.0 (100 randomly sampled tasks)}
\label{tab:java-transfer}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Success Rate} & \textbf{Compilation Rate} & \textbf{Relevant File Rate} \\
\midrule
Qwen-8B (Base) & 4.2\% & 23.8\% & 31.4\% \\
Qwen-8B (Python SFT) & 3.8\% & 21.2\% & 28.9\% \\
Qwen-8B (Python RL) & \textbf{8.7\%} & \textbf{34.5\%} & \textbf{42.8\%} \\
\midrule
Qwen-32B (Base) & 6.1\% & 28.4\% & 36.7\% \\
Qwen-32B (Python SFT) & 5.9\% & 26.8\% & 35.2\% \\
Qwen-32B (Python RL) & \textbf{12.4\%} & \textbf{41.6\%} & \textbf{49.3\%} \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Positive Transfer Evidence}

The results demonstrate clear positive transfer from Python RL training to Java debugging:

\begin{itemize}
\item \textbf{Success Rate Improvement}: RL-trained models achieve 2x higher success rates than baselines on Java tasks
\item \textbf{Compilation Success}: Agents generate syntactically valid Java code at higher rates, indicating transfer of programming language understanding
\item \textbf{Problem Localization}: Higher rates of identifying correct files for modification suggest transfer of debugging strategy
\end{itemize}

\subsubsection{Transfer Mechanism Analysis}

The positive transfer appears to operate through several mechanisms:

\textbf{Strategic Transfer}: Repository exploration strategies learned in Python (systematic file navigation, dependency analysis, test examination) transfer effectively to Java projects despite syntactic differences.

\textbf{Tool Usage Patterns}: Effective patterns for using grep, find, and other basic tools generalize across programming languages and project structures.

\textbf{Debugging Methodology}: High-level debugging approaches (understanding error messages, locating bug symptoms, verifying fixes) show strong cross-language transfer.

\textbf{Error Recovery}: Robust error handling and strategy adaptation learned during RL training benefit Java debugging even when specific approaches fail.

\subsection{General Code Generation: HumanEval Results}
...
\textbf{RQ1 (Agent-in-the-Loop Effectiveness)}: Agent-in-the-loop RL produces statistically significant improvements (7-10 percentage points) over both pretrained models and supervised fine-tuning, validating the core hypothesis.

\textbf{RQ2 (Scaffold Complexity Effects)}: Minimalist scaffolds achieve performance parity with feature-rich alternatives while requiring significantly less computational resources, supporting the bitter lesson hypothesis in software engineering domains.

\textbf{RQ3 (Generalization and Transfer)}: Debugging skills learned through RL training demonstrate positive transfer to both cross-language debugging (2x improvement on Java tasks) and general code generation (4\% improvement on HumanEval).
...
\begin{itemize}
\item \textbf{Paradigm Validation}: Agent-in-the-loop training represents a viable alternative to static supervised learning for interactive programming tasks
...
\item \textbf{Practical Feasibility}: Academic research groups can achieve state-of-the-art results with open-source infrastructure and reasonable computational budgets
