...
Table~\ref{tab:main-results} presents our primary experimental results on SWE-Bench-Verified, comparing interactive RL training against multiple baseline approaches.
...
These results provide strong evidence that interactive RL produces substantial and statistically significant improvements over both pre-trained models and supervised fine-tuning approaches.
...
\section{RQ3: SWE-Gym Training vs.
  SWE-Bench/Multi-SWE-Bench Evaluation} \label{sec:rq3-overfit}

We test for overfitting by training on \ac{SWE-Gym} and evaluating on \ac{SWE-Bench} and Multi-SWE-Bench.
...
\textbf{RQ3 (Cross-Dataset Generalization)}: Training on SWE-Gym yields improvements on \ac{SWE-Bench} and Multi-SWE-Bench without clear overfitting. \todoinline{Add quantitative results and CIs from Sec.~\ref{sec:rq3-overfit}.}
