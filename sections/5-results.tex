...
We begin by validating training convergence (RQ0), then examine scaffold adaptation (RQ1), SWE-Bench-Verified performance (RQ2), and multilingual generalization (RQ3).

\section{RQ0: Training Convergence?}
\label{sec:training-convergence}

\textbf{Research Question:}
Does \ac{GSPO} training converge effectively with execution-free rewards?

Training proceeded for a little over 2 epochs over the 1{,}000-task curriculum, completing in 2 days of wall-clock time on 3 A100 \acp{GPU} (144 total \ac{GPU}-hours), with \Cref{fig:reward-progression} demonstrating sustained learning throughout.
...
While this exploration maintains healthy gradient signal, extended training would likely require curriculum mixing with non-agentic or meaningfully different tasks to preserve broader model capabilities and prevent catastrophic forgetting.

\section{RQ1: Nano Agent Scaffold Adaptation}
\label{sec:rq1-harness}

\textbf{Research Question:}
How does \ac{GSPO} training improve Nano agent scaffold adaptation?

Effective scaffold adaptation requires the policy to generate valid tool calls, strategically appropriate commands, and applying syntactically correct changes to files.
...
One plausible explanation is that the policy adopted a strategy of broadly affecting the repository to attain tail rewards, but this approach proved harmful to performance and was eventually superseded after many gradient updates.
