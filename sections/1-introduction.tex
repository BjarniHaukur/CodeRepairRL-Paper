...
Our work is complementary: we also use execution-free patch-similarity rewards, but combine them with \ac{GSPO}—a more robust sequence-level variant—in an agentic, multi-step regime where the model must first acquire its own context through tool use before proposing edits.
...
Terminal-oriented agency benchmarks such as TauBench~\cite{taubench2024} provide standardized evaluation frameworks for such capabilities; our cross-scaffold transfer experiments (\cref{sec:rq3-scaffold}) further explore generalization across different interaction interfaces.
...
\begin{itemize}
	\item Core contribution: evaluate zero-shot and few-shot transfer on Mini-SWE-Agent-, Aider-, and OpenHands-style harnesses without retraining.\todoinline{Specify exact harness configurations once experiments finish.}
...
We employ restricted file access and process-level isolation to ensure safe episode execution without compromising host system integrity.
...
We employ \ac{GSPO}~\cite{gspo2025}, a sequence-level policy optimization algorithm that extends the group-relative baseline principle with improved importance weighting.
\ac{GSPO} demonstrates superior robustness and stability compared to earlier group-relative methods, proving particularly effective for the small effective batch sizes and high-variance rewards inherent in multi-turn agent training under academic compute constraints.
...
\textbf{Model Architecture Scope}: Our primary development and analysis focuses on the base Qwen3 model family—hybrid reasoning models with strong tool-calling capabilities—with reasoning explicitly disabled to conserve context window budget in multi-turn episodes.
We use Qwen3 (not the specialized Qwen3-Coder variant) for its superior tool-calling reliability and open availability.
