...
To make this investigation concrete, we organize the work around four learnings (RQ1-RQ4) that arise from building an open-source agentic model in a single minimalist harness (Nano).
\todoinline{Verify RQ numbering and titles are consistent with Chapters 3 and 5 before submission.}

\textbf{RQ1: Does online \ac{RL} improve debugging capabilities through agent-harness adaptation?
}
\begin{itemize}
	...
	\item Hypothesis: \ac{RL} improves harness adaptation reflected by increased tool success and solvency rates.
\end{itemize}
...
\begin{itemize}
	\item Goal: validate that our training recipe transfers beyond the initial base model
	\item Design: replicate key experiments on alternative open models (e.g., SmolLM3) without bespoke engineering
	\item Purpose: test whether findings reflect general principles for online \ac{RL} training rather than model-specific tuning
\end{itemize}

\textbf{RQ4: Do learned debugging skills transfer to different harnesses and tasks?}
\begin{itemize}
	\item Cross-harness (zero-shot): evaluate with Mini-SWE-Agent and OpenHands without adaptation
	\item Cross-task: evaluate on TauBench to test tool-calling skill transfer
\end{itemize}
...
We employ \ac{GRPO}, a variant of \ac{PPO}~\cite{ppo2017} that forgoes value function estimation in favor of group-relative reward baselines.
...
We maintain identical model architectures within each comparison group, training data sources, and computational budgets across all experimental conditions.
...
We employ both the SWE-Gym environment for training, which provides thousands of real-world Python bug-fixing tasks, and SWE-Bench for evaluation, ensuring robust assessment on human-verified bug fixes.
