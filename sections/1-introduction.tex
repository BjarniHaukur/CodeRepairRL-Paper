...
The models possess sufficient latent understanding to generate meaningful action sequences and interpret environmental responses, providing the crucial foundation upon which \ac{RL} optimization can build.
...
Systems like Claude 3.5 Sonnet demonstrated unprecedented coding performance (improving from 33.4\% to 49.0\% on \ac{SWE-Bench-Verified}) and introduced revolutionary "computer use" capabilities, while OpenAI's o1 showed remarkable reasoning improvements through \ac{RL} techniques.
These breakthroughs likely reflect large-scale deployment of similar scaffolded training approaches by major \ac{AI} laboratories, validating the potential of this paradigm for developing genuinely agentic \ac{AI} systems.
...
Recent investigations into agent-based approaches have demonstrated the potential of tool-augmented \acp{LLM} for software engineering tasks \cite{openHands2024, aider2024}.
...
The infrastructure supports both Group Relative Policy Optimization (\ac{GRPO}) and can be extended to other policy gradient methods.
...
\begin{itemize}
	...
	\item \todoinline{Insert concrete numbers and plots from training logs: tool success, invalid calls, retries, length-of-trajectory, \ac{KL} to reference prompts.}
\end{itemize}
...
This research employs a rigorous experimental methodology designed to systematically evaluate \ac{RL} for automated program repair.
...
\textbf{Chapter 2: Background and Theoretical Foundations} establishes the theoretical context for our work, reviewing \ac{RL} algorithms for language models with particular emphasis on Group Relative Policy Optimization.
