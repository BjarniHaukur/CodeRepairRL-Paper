...
Per Monperrus guidelines, 30+ references required with recent academic work.
}
...
\todoinline{Per Monperrus guidelines, Problem Statement should have at least two paragraphs: 1. what is the problem you solve? 2. why is it an important problem?
Consider restructuring to clearly separate problem definition from importance/motivation.
}
...
We demonstrate that static patch-similarity rewards can improve repository navigation and repair performance without test execution, and that the resulting behaviors transfer across base models and from training benchmarks to held-out evaluations.
...
\todoinline{Add explicit "Contributions" subsection here with bullet list format per Monperrus guidelines: "Contributions: A bullet list of the contributions to the state-of-the-art, with one novelty explanation per point."}
...
\begin{itemize}
	...
	\item Hypothesis: execution-free patch-similarity rewards enable unified training across programming languages without language-specific adaptations, with measurable improvements on held-out multilingual tasks despite limited non-Python training data.
\end{itemize}
...
\textbf{Chapter 5: Results} presents experimental findings: harness adaptation metrics and behavioral changes (RQ1), performance comparisons on SWE-Bench-Verified (RQ2), language-agnostic training effectiveness on the multilingual held-out subset (RQ3), training dynamics and convergence, and qualitative trajectory examination with statistical analyses.
