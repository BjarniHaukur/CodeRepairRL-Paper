...
\vspace{3cm}
\end{minipage}}
\caption{Agent-in-the-loop reinforcement learning architecture showing the complete feedback cycle between language model policy, agent scaffold, repository environment, and reward computation. The diagram illustrates how traditional RL training is enhanced by embedding interactive agents directly within the optimization loop.}
...
\vspace{3cm}
\end{minipage}}
\caption{Training dynamics showing the evolution of reward and success rate throughout GRPO training for both 8B and 32B parameter Qwen models using the nano-agent scaffold. The curves demonstrate monotonic improvement and remarkable training stability compared to traditional PPO implementations.}
...
\vspace{3cm}
\end{minipage}}
\caption{Performance comparison on SWE-Bench-Verified showing statistically significant improvements from agent-in-the-loop RL training over both pretrained models and supervised fine-tuning approaches. Error bars represent 95\% confidence intervals.}
...
\vspace{3cm}
\textbf{Figure Placeholder}\\[0.5cm]
Cross-Language Transfer Results\\[0.5cm]
\textit{This figure should visualize:}\\
\textit{• Java debugging performance comparison}\\
\textit{• Transfer learning effectiveness metrics}\\
\textit{• Breakdown by success rate, compilation rate, file identification}\\
\textit{• Comparison between Python-trained models and baselines}\\
\textit{• Evidence of positive transfer mechanisms}\\
\vspace{3cm}
\end{minipage}}
\caption{Cross-language generalization results showing positive transfer from Python RL training to Java debugging tasks. The results demonstrate that debugging strategies learned through agent-in-the-loop training generalize across programming languages.}
\label{fig:cross-language-transfer}
\end{figure}
...
\textit{• Comparison of nano-agent vs. feature-rich scaffold behaviors}\\
